...
import { getHealthAdviceById, updateHealthAdviceById } from 'apiSdk/health-advices';
...
function HealthAdviceEditPage() {
  ...
  const handleSubmit = async (values: HealthAdviceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateHealthAdviceById(id, values);
      mutate(updated);
      resetForm();
    } catch (error) {
      setFormError(error);
